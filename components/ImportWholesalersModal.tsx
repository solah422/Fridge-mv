...
type ParsedWholesaler = Omit<Wholesaler, 'id'>;

interface ImportWholesalersModalProps {
  isOpen: boolean;
  onClose: () => void;
  onImport: (wholesalers: ParsedWholesaler[]) => void;
}

export const ImportWholesalersModal: React.FC<ImportWholesalersModalProps> = ({ isOpen, onClose, onImport }) => {
  const [file, setFile] = useState<File | null>(null);
  const [parsedData, setParsedData] = useState<ParsedWholesaler[]>([]);
  ...
  const handleDownloadSample = () => {
    const headers = "name,contactPerson,contactNumber,email";
    const sampleData = "Global Foods Inc.,John Doe,555-1234,john.d@globalfoods.com";
    const csvContent = "data:text/csv;charset=utf-8," + headers + "\n" + sampleData;
    const encodedUri = encodeURI(csvContent);
    const link = document.createElement("a");
    link.setAttribute("href", encodedUri);
    link.setAttribute("download", "wholesaler_import_sample.csv");
  ...
  const parseCSV = (csvFile: File) => {
    ...
    reader.onload = (event) => {
        ...
        const newWholesalers: ParsedWholesaler[] = [];
        ...
        for (let i = 1; i < lines.length; i++) {
            ...
            if (!name) { 
                ...
                newWholesalers.push({ name, contactPerson, contactNumber, email });
            }
        }
        setErrors(newErrors);
        setParsedData(newWholesalers);
    }
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-40">
      <div className="bg-[rgb(var(--color-bg-card))] rounded-lg shadow-xl w-full max-w-3xl max-h-[90vh] flex flex-col">
        <div className="p-4 border-b border-[rgb(var(--color-border-subtle))] flex justify-between items-center">
          <h3 className="text-xl font-bold text-[rgb(var(--color-text-base))]">Bulk Import Wholesalers</h3>
        ...
        <div className="p-6 space-y-4 overflow-y-auto flex-grow">
          <div className="p-4 bg-[rgb(var(--color-bg-subtle))] rounded-md border border-[rgb(var(--color-border-subtle))]">
            ...
            <ol className="list-decimal list-inside text-sm text-[rgb(var(--color-text-muted))] mt-2 space-y-1">
              ...
              <li>Fill the sheet with your wholesaler data. The `name` column is required.</li>
              <li>`contactPerson`, `contactNumber`, and `email` are optional.</li>
          ...
          <div>
            <label htmlFor="csv-upload-wholesaler" className="block text-sm font-medium text-[rgb(var(--color-text-muted))] mb-1">Upload CSV File</label>
            <input 
                id="csv-upload-wholesaler"
                type="file"
                accept=".csv"
          ...
          {parsedData.length > 0 && errors.length === 0 && (
            <div>
              <h4 className="font-semibold text-[rgb(var(--color-text-base))] mb-2">Import Preview ({parsedData.length} wholesalers)</h4>
                ...
                <table className="min-w-full divide-y divide-[rgb(var(--color-border-subtle))] text-sm">
                  <thead className="bg-[rgb(var(--color-bg-subtle))] sticky top-0">
                    <tr>
                      ...
                      <th className="px-3 py-2 text-left font-medium text-[rgb(var(--color-text-muted))]">Contact Person</th>
                      <th className="px-3 py-2 text-left font-medium text-[rgb(var(--color-text-muted))]">Contact Number</th>
                    </tr>
                  </thead>
                  <tbody className="bg-[rgb(var(--color-bg-card))] divide-y divide-[rgb(var(--color-border-subtle))]">
                    {parsedData.slice(0, 10).map((p, i) => (
                      <tr key={i}>
                        ...
                        <td className="px-3 py-2 whitespace-nowrap text-[rgb(var(--color-text-muted))]">{p.contactPerson || 'N/A'}</td>
                        <td className="px-3 py-2 whitespace-nowrap text-[rgb(var(--color-text-muted))]">{p.contactNumber || 'N/A'}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
